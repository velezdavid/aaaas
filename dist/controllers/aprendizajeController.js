...
router.get("/:id", async (req, res) => {
  try {
    ...
    if (aprendizaje) {
      ...
      res.status(404).json({
        message: "Registro de Aprendizaje no encontrado"
      });
    }
  } catch (err) {
...
router.put("/:id", async (req, res) => {
  try {
    const aprendizaje = await Aprendizaje.findByIdAndUpdate(req.params.id, req.body, {
      new: true
    });
    if (aprendizaje) {
      res.status(200).json(aprendizaje);
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    if (aprendizaje) {
      res.status(200).json({
        message: "Registro de Aprendizaje eliminado exitosamente"
      });
    } else {
      res.status(404).json({
        message: "Registro de Aprendizaje no encontrado"
