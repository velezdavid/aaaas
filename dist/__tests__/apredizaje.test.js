...
describe("AprendizajeController", () => {
  beforeAll(async () => {
    await mongoose.connect("mongodb+srv://facci:facci@cluster0.kidus.mongodb.net/aprendizajes", {
  ...
  describe("POST /api/aprendizajes", () => {
    it("debería crear un nuevo registro", async () => {
      const date = new Date();
      const response = await request(app).post("/api/aprendizajes").send({
        ID_Idioma: 1,
        ID_Instructor: 1,
        Fecha: "2023-01-01",
        Hora: "14:20",
  ...
  describe("GET /api/aprendizajes", () => {
    it("debería devolver una lista de registros", async () => {
      const response = await request(app).get("/api/aprendizajes").expect("Content-Type", /json/).expect(200);
